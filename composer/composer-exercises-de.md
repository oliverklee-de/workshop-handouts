...
[zum Cheatsheet](/cheatsheets/composer-cheatsheet-de.md)

## Installation

1. Prüft, dass PHP lokal installiert ist und im Pfad liegt, und welche Version
   installiert ist.
2. Geht auf https://getcomposer.org/ und schaut, was beim Neuladen der Seite
   passiert.
3. Installiert euch Composer so, dass er als `composer`
   (nicht: `composer.phar`) bei euch im Pfad liegt.
4. Lasst euch anzeigen, welche Version von Composer ihr installiert habt.
5. Lasst euch die generelle Composer-Hilfe anzeigen.
6. Lasst euch die Liste aller Composer-Befehle anzeigen.
7. Lasst euch die Composer-Hilfe zu `selfupdate` anzeigen.
8. Lasst ein Composer-Update laufen.
...
2. Schaut euch die generierte `composer.lock` an und schaut euch im Verzeichnis
   `vendor/` um.
3. Lasst euch anzeigen, welche Pakete jetzt alle installiert sind.
4. Bindet den Composer-Autoloader in `start.php` ein und führt die Datei erneut
   aus.
5. Benutzt die Timer-Klasse in der `start.php` ein und lasst die Zeit
   von `sleep(1);` stoppen und ausgeben.
6. Findet die Dev-Abhängigkeiten von `phpunit/php-timer` heraus (entweder
   lokal in `vendor` oder auf Packagist) und schaut nach, ob diese installiert
   sind.
7. Installiert `squizlabs/php_codesniffer` als Dev-Abhängigkeit, lest euch die
...
8. Lest euch die
   ...
   durch.
9. Tragt ein, dass euer Projekt mindestens PHP 7.0 und maximal PHP 7.2.x hat.
   Lass ein `composer update` laufen, um zu schauen, ob das Requirement
   funktioniert.
10. Ändert die Dev-Abhängigkeit `squizlabs/php_codesniffer` so, dass alle
    non-breaking Updates ab Version 3.0.0 erlaubt sind.
11. Aktualisiert die Abhängigkeiten und schaut, welche Version von
    `squizlabs/php_codesniffer` installiert ist.
12. Aktualisiert mit `composer update` die Abhängigkeiten so, dass die
...
13. Ändert die Dev-Abhängigkeit von `squizlabs/php_codesniffer` so, dass nur
    Bugfix-Updates zu Version 3.0.0 erlaubt sind.
14. Aktualisiert die Abhängigkeiten und schaut, welche Version von
    `squizlabs/php_codesniffer` installiert ist.
15. Schaut euch auf Packagist die Abhängigkeiten von `phplist/core`,
...
2. Überlegt euch einen Vendor- und Produkt-PHP-Namespace für euer
   Übungsprojekt.
3. Legt im Unter-Namespace `Service` eine Klasse `Greeter` an, die lediglich
   eine
...
5. Lest euch die folgenden Standards durch:
    - PSR-4
    - PSR-1
    - PSR-2
    - PSR-12
...
2. Installiert das Paket `friendsofphp/php-cs-fixer` global.
3. Kopiert euch bestehendes Projekt lokal auf die Platte.
4. Erzeugt darin ein neues lokales-Git-Repository und committet alle Dateien.
...
1. Erzeugt ein neues Projekt aus `phplist/base-distribution` und schaut an, was
   dabei herauskommt.
2. Erzeugt ein neues Projekt aus `helhum/typo3-distribution` und erzeugt dabei
...
3. Aktualisiert die Pakete rekursiv mit allen Abhängigkeiten.
4. Findet heraus, warum das Paket `doctrine/inflector` installiert ist (mit
   der kompletten Abhängigkeitskette).
5. Überprüft, ob die `composer.json` valide ist.
...
1. Legt in eurem Übungsproject ein Verzeichnis `packages/greeter` an und legt
   ...
   noch funktioniert.
2. Tragt das GitHub-Repository von `phpunit/php-timer` als VCS-Repository in
...
1. Führt in dem phpList-Distribution-Projekt das Composer-Skript
   `list-modules` aus.
2. Schaut euch die [Composer-Skripte von Emogrifier
   ](https://github.com/MyIntervals/emogrifier/blob/master/composer.json)
   an.
3. Installiert in eurem Projekt, über das ihr `php-cs-fixer` habt laufen
   lassen, die folgenden Pakete als Dev-Abhängigkeiten:
    - PHP_CodeSniffer
    - PHPMD
    - PHPStan
4. Richtet euch Composer-Scripte mit diesen Namen ein, die euren Code checken:
    - `ci:static:lint` (ruft `php -l` auf eurem Sourcecode auf)
    - `ci:static:sniff`
    - `ci:static:phpmd`
    - `ci:static:phpstan`
5. Richtet euch ein Composer-Script `ci:static` ein, das die anderen drei
   Scripts nacheinander aufruft.
6. Schaut euch in dem phpList-Distribution-Projekt das Composer-Script
   `list-modules` sowie die entsprechende PHP-Klasse an.
7. Schreibt euch in eurem Übungsprojekt ein Composer-Script, das `php_uname`
   per `echo` ausgibt.
